 ...
 */
$this->breadcrumbs = array('Аудиоальбомы');
?>
<?php if(Access::canEdit()): ?>
<div class="buttons">
    <i class="icon" id="album_icon_audio"></i>
    <div class="m_button">
        <?php echo CHtml::link('Добавить аудиоальбом', Yii::app()->createUrl('/gallery/album/add_audio', array('gameId' => Yii::app()->user->getGameId())), array('id' => 'add_album', 'class' => 'fancybox.ajax btn2')); ?>
    </div>
</div>
<?php endif; ?>
<ul class="album">
        ...
        <li>
            <div class="body">
                <?php $this->renderPartial('webroot.themes.'.Yii::app()->theme->name.'.views.modules.gallery.common.album', array(
                    'model' => $model,
                    'route' => '/gallery/album/show_audio',
                    'routeParams' => array('gameId' => $model->user->game_id)
                )); ?>
            </div>
            <div class="title">
                <i class="icon" id="album"></i>
                <?php echo CHtml::link($model->title, Yii::app()->createUrl('/user/show/show_audio', array('album_id' => $model->id, 'gameId' => $model->user->game_id)))?>
