...
<div class="buttons">
    ...
    <div class="m_button">
        <?php echo CHtml::link('Добавить видеоальбом', Yii::app()->createUrl('/community/album/video_add', array('community_alias' => Yii::app()->community->alias)), array('id' => 'add_album', 'class' => 'fancybox.ajax btn2')); ?>
    </div>
</div>
<?php endif; ?>
<ul class="album">
        ...
        <li>
            <div class="body">
                <?php $this->renderPartial('webroot.themes.'.Yii::app()->theme->name.'.views.modules.gallery.common.album', array(
                    'model' => $model,
                    'route' => '/community/album/video_show',
                    'routeParams' => array('community_alias' => $model->community_alias)
                )); ?>
            </div>
            <div class="title">
                <i class="icon" id="album"></i>
                <?php echo CHtml::link($model->title, Yii::app()->createUrl('/community/album/video_show', array('album_id' => $model->id, 'community_alias' => $model->community_alias)))?>
