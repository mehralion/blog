...
<?php echo $form->dropDownListRow(
    ...
    array(
        ...
        'hint' => 'В ТОП рейтингах участвуют только те видео, которые доступны всем'
    )); ?>
...
<script>
    $(function () {
        $(document.body).on('change', '#add-album-form #GalleryAlbumVideo_image_front', function () {
            if ($(this).val() != '')
                $('#add-album-form .field').val($(this).val());
        });
        $(document.body).on('click', '#add-album-form .obzor', function () {
            $('#GalleryAlbumVideo_image_front').trigger('click');
        });
    });
</script>