 ...
 *
 * @var EventCommentPost $model
 */?>
<article class="long_block">
    <div class="content event_comment">
        <?php echo $model->user->getFullLogin(); ?> добавил комментарий к заметке
        <?php echo $model->userOwner->getFullLogin(); ?>
        <?php echo CHtml::link(
            $model->info->title,
            Yii::app()->createUrl(
                '/community/post/show',
                array('id' => $model->info->item_id, 'comment_id' => $model->comment_id , '#' => 'comment_'.$model->comment_id,'community_alias' => $model->info->community_alias)
            )); ?>
    ...
    <div class="info">
        ...
        <div class="right">
            <span class="ratingCount ajax"><?php echo $model->comment->canRate(); ?>
                <span class="showRate ajax" data-link="<?php echo Yii::app()->createUrl('/community/listrate/comment_post', array('id' => $model->comment_id, 'community_alias' => $model->info->community_alias)); ?>" data-html="true" rel="tooltip" title="<?php echo $model->comment->getRateList(); ?>">Понравилось
