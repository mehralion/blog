...
*/
	...
	protected function buildOfflinePages($docPath,$themePath)
	{
		...
		foreach($this->classes as $name=>$class)
		{
            ...
            if($fileName !== null)
                file_put_contents($fileDir.'/'.$fileName.'.html',$content);
            else
	...
	public function renderInheritance($class)
	{
        ...
        foreach($parents as $key =>$value)
            $parents[$key] = $this->changeNS($value);

		foreach($class->parentClasses as $parent)
		{
			if(isset($this->classes[$parent]))
				$parents[]='{{'.$parent.'|'.$this->changeNS($parent).'}}';
			else
				$parents[]=$this->changeNS($parent);
		}
	...
	public function renderImplements($class)
	{
		...
		foreach($class->interfaces as $interface)
		{
			if(isset($this->classes[$interface]))
				$interfaces[]='{{'.$interface.'|'.$this->changeNS($interface).'}}';
			else
				$interfaces[]=$this->changeNS($interface);
		}
	...
	public function renderSubclasses($class)
	{
		...
		foreach($class->subclasses as $subclass)
		{
			if(isset($this->classes[$subclass]))
				$subclasses[]='{{'.$subclass.'|'.$this->changeNS($subclass).'}}';
			else
				$subclasses[]=$this->changeNS($subclass);
		}
	...
	protected function fixOfflineLink($matches)
	{
		if(($pos=strpos($matches[1],'::'))!==false)
		{
			...
			return "<a href=\"/docs/api/{$className}.html#{$method}\">{$matches[2]}</a>";
		}
		else
			return "<a href=\"/docs/api/{$matches[1]}.html\">{$matches[2]}</a>";
	}
