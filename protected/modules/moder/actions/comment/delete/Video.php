...
class Video extends \CAction
{
    public function run()
    {
        ...
        /** @var \CommentItemVideo $CommentItem */
        $CommentItem = \CommentItemVideo::model()->find($criteria);
        ...
        if($post) {
            ...
            try {
                ...
                if(!$CommentItem->delete(true, null, true)) {
                    $error = true;
                ...
                $GalleryVideo = \GalleryVideo::model()->findByPk($CommentItem->item_id);
                if($GalleryVideo->comment_count > 0)
                    $GalleryVideo->comment_count -= 1;
                $GalleryVideo->update_datetime = \DateTimeFormat::format();
