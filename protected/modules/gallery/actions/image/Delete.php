...
class Delete extends GalleryAction
{
    public function run()
    {
        ...
        elseif(($this->isCommunity && !\Yii::app()->community->isModer() && !$Image->user_id != \Yii::app()->user->id)
            || (!$this->isCommunity && $Image->user_id != \Yii::app()->user->id))
            \Yii::app()->message->setErrors('danger', 'Вы не можете удалить эту фотографию!');
        else {
            $Image->is_deleted = 1;
            $Image->user_deleted_id = \Yii::app()->user->id;
            $Image->scenario = 'deleteImage';
            if($Image->delete())
                \Yii::app()->message->setText('success', 'Фотография удалена!');
            else
                \Yii::app()->message->setErrors('danger', 'Во время удаления возникла ошибка, попробуй позже!');
        }
