...
{
    public $viewName = 'video/form';

    public function run()
    {
        $Album = new \GalleryAlbumVideo('create');
        ...
        if($this->isCommunity) {
            ...
            $this->successLinkRoute = '/community/album/video_show';
            $this->successLinkParams = array('community_alias' => \Yii::app()->community->alias);
        } else {
            $this->successLinkRoute = '/gallery/album/show_video';
            $this->successLinkParams = array('gameId' => \Yii::app()->user->getGameId());
        }

        $post = \Yii::app()->request->getParam('GalleryAlbumVideo');
        if(!empty($post)) {
