...
class Add extends GalleryAction
{
    ...
    public function run()
    {
        $Album = new \GalleryAlbumImage('create');
        ...
        if($this->isCommunity) {
            ...
            $this->successLinkRoute = '/community/album/image_show';
            $this->successLinkParams = array('community_alias' => \Yii::app()->community->alias);
        } else {
            $this->successLinkRoute = '/gallery/album/show_image';
        ...
        if(!empty($post)) {
            ...
            $error = false;
            //Транзакция, если обложка не загрузилась, не добавляем альбом
